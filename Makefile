BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = "Reisen_und_Abenteuer_im_Apachenlande"
TITLE_NAME = "Reisen und Abenteuer im Apachenlande"
...
PREFACE_EPUB = chapters/preface/preface_epub.md chapters/preface/00_Verzeichniss_der_Illustrationen_epub.md
PREFACE_HTML_PDF = chapters/preface/00_Verzeichniss_der_Illustrationen_pdf_html.md
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA) $(CHAPTERS) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Abenteuer_im_Apachenlande_*.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Abenteuer_im_Apachenlande_*.jpg
	rm cover1.jpg 
	rm logo.jpg
